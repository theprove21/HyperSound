...
class Net(nn.Module):
    def __init__(self, device):
        ...
        self.manifolds = [PoincareBall(c=Curvature(0.1, requires_grad = True)), # Manifold for the conv layers
                         PoincareBall(c=Curvature(0.1, requires_grad = True)),
                         PoincareBall(c=Curvature(0.1, requires_grad = True)),
                        #  PoincareBall(c=Curvature(0.1, requires_grad = True)),
                         PoincareBall(c=Curvature(0.1, requires_grad = True))] # Manifold for the linear layers
        
        self.switch_manifold1 = ChangeManifold(self.manifolds[1])
        self.switch_manifold2 = ChangeManifold(self.manifolds[2])
        self.switch_manifold3 = ChangeManifold(self.manifolds[3])
        # self.switch_manifold4 = ChangeManifold(self.manifolds[4]) # Here we only require to change to manifolds[1]
        ...
        self.conv2 = hnn.HConvolution2d(in_channels=24, out_channels=36, kernel_size=4, padding=0, manifold=self.manifolds[1])
        # self.conv2 = nn.Conv2d(in_channels=24, out_channels=36, kernel_size=4, padding=0)
        self.conv3 = hnn.HConvolution2d(in_channels=36, out_channels=48, kernel_size=3, padding=0, manifold=self.manifolds[2])
        # self.conv3 = nn.Conv2d(in_channels=36, out_channels=48, kernel_size=3, padding=0)
        self.avg_pool = hnn.HAvgPool2d(kernel_size=(17, 17), manifold=self.manifolds[2])
        self.max_pool1 = hnn.HMaxPool2d(kernel_size=(3,3), stride=3, manifold = self.manifolds[0])
        self.max_pool2 = hnn.HMaxPool2d(kernel_size=(2,2), stride=2, manifold = self.manifolds[1])
        self.relu1 = hnn.HReLU(manifold=self.manifolds[0])
        self.relu2 = hnn.HReLU(manifold=self.manifolds[1])
        self.relu3 = hnn.HReLU(manifold=self.manifolds[2])

        ###################################################

        ################# LINEAR LAYERS ###################
        # self.fc1 = nn.Linear(in_features=48, out_features=60)
        self.fc1 = hnn.HLinear(in_features=48, out_features=60, manifold=self.manifolds[3])
        # self.fc2 = nn.Linear(in_features=60, out_features=10)
        self.fc2 = hnn.HLinear(in_features=60, out_features=10,manifold= self.manifolds[3])
        self.relu4 = hnn.HReLU(manifold=self.manifolds[3])
        ###################################################
    ...
    def forward(self, x):
        ...
        x = self.manifolds[3].logmap(x=None, y= x)
